	...
	while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'v':				/* v, vn, vt */
			...
			case '\0':			/* vertex */
				...
				fgets(buf, sizeof(buf), file);
				ss.str(buf);
				ss >> vertices[3 * numvertices + 0] >> vertices[3 * numvertices + 1] >> vertices[3 * numvertices + 2];
			...
			case 'n':				/* normal */
				...
				fgets(buf, sizeof(buf), file);
				ss.str(buf);
				ss >> normals[3 * numnormals + 0] >> normals[3 * numnormals + 1] >> normals[3 * numnormals + 2];
				/*
									fscanf(file, "%f %f %f", 
			...
			case 't':				/* texcoord */
				...
				fgets(buf, sizeof(buf), file);
				ss.str(buf);
				ss>>texcoords[2 * numtexcoords + 0]>>texcoords[2 * numtexcoords + 1];
				/*
									fscanf(file, "%f %f", 
...
#ifdef _USE_FACE_MATERIAL_
					T(numtriangles).matindex = material;
#endif // _USE_FACE_MATERIAL_
